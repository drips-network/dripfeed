...
export class BlockFetcher {
  ...
  async fetch(): Promise<FetchResult | null> {
    ...
    const totalEvents = await withDbRetry(async () => {
      ...
      try {
        ...
        const logs = await this._rpc.getLogs(this._decoder.contractAddresses, fromBlock, toBlock);
        const blockSummaries = await this._collectBlockSummaries(logs, fromBlock, toBlock, safeBlock);
        ...
        for (const logItem of logs) {
          ...
          if (decodeResult.reason === 'decode_error' || decodeResult.reason === 'missing_log_fields') {
  ...
  private async _collectBlockSummaries(
    ...
    const minReorgWindow: number = this._confirmations === 0 ? MIN_HASH_WINDOW : this._confirmations;
